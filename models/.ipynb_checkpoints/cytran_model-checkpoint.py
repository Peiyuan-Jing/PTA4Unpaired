...
class CyTranModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--ngf_cytran', type=int, default=64, help='number of down')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.AdamW(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.9))
            self.optimizer_D = torch.optim.AdamW(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.9))
