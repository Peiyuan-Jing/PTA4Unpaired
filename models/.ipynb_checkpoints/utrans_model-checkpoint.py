...
class UTransModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--depth', type=int, default=6, help='number of down')
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, depth=opt.depth, heads=opt.heads, img_size=opt.vit_img_size)
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), lr=5*opt.lr, betas=(opt.beta1, 0.999))
    ...
    def test_feature(self):
        self.netG_A(self.real_A)
        self.netG_B(self.real_B)
        ##for syngan
    # def backward_D_basic(self, netD, real, fake):
    #     """Calculate GAN loss for the discriminator using MSE loss"""
    #     # Real
    #     pred_real = netD(real)
    #     loss_D_real = self.MSE_loss(pred_real, self.target_real) * 1
        
    #     # Fake
    #     pred_fake = netD(fake.detach())
    #     loss_D_fake = self.MSE_loss(pred_fake, self.target_fake) * 1
    ...
    #     lambda_idt = 0#self.opt.lambda_identity
    ...
    #     self.loss_G_A = self.MSE_loss(self.netD_A(self.fake_B), self.target_real) * 1
    #     self.loss_G_B = self.MSE_loss(self.netD_B(self.fake_A), self.target_real) * 1
        
    #     # L1 loss (pixel-wise loss)
    #     self.loss_L1_A = self.L1_loss(self.fake_B, self.real_B) * 100
    #     self.loss_L1_B = self.L1_loss(self.fake_A, self.real_A) * 100
